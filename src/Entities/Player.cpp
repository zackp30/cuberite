...
void cPlayer::SetTouchGround(bool a_bTouchGround)
{
	...
	*/
	...
	if (
		...
		(
			(
				((GetPosY() >= 1) && ((GetPosY() - POSY_TOINT)) <= std::numeric_limits<double>::epsilon()) &&
				(
			...
			(
				((GetPosY() >= POSY_TOINT) && ((GetPosY() - (POSY_TOINT + 0.5)) <= std::numeric_limits<double>::epsilon()) &&
				(
			...
			(
				(fmod(GetPosY(), 0.125) <= std::numeric_limits<double>::epsilon()) &&
				(
	...
	/* Unfortunately whatever the reason, there are still desyncs in on-ground status between the client and server. Por ejemplo:
