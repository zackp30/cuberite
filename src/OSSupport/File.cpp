...
int cFile::Read (void * a_Buffer, size_t a_NumBytes)
{
	...
	return static_cast<int>(fread(a_Buffer, 1, a_NumBytes, m_File));  // fread() returns the portion of Count parameter actually read, so we need to send a_NumBytes as Count
}
...
int cFile::Write(const void * a_Buffer, size_t a_NumBytes)
{
	...
	int res = static_cast<int>(fwrite(a_Buffer, 1, a_NumBytes, m_File));  // fwrite() returns the portion of Count parameter actually written, so we need to send iNumBytes as Count
	return res;
}
